        ...
        if len(df.columns) >= 2:
            ...
            if total_count > 0:
                ...
                with col2:
                    ...
                    st.write(f"Scanned {final_count} items.")
                    st.divider()
                    ...
                    with c1:
                        ...
                        if final_cuisine_tags:
                            # Display all cuisine matches (Limited to 6 as requested)
                            cuisine_list = sorted(list(final_cuisine_tags))[:6]
                            for c in cuisine_list: st.success(c)
                        else:
                            st.write("N/A")
                    ...
                    with c3:
                        # --- MODIFIED ACCURACY FILTER FOR SUBPAGES ---
                        subpages = []
                        # We only use the Cuisine Tags as references to keep it accurate
                        cuisine_refs = [str(x).lower() for x in list(final_cuisine_tags)]
                        
                        for t in clean_tags:
                            if "Subpage" in str(t):
                                t_low = str(t).lower()
                                # Only add subpage if it matches one of the identified cuisines
                                if any(ref in t_low for ref in cuisine_refs if len(ref) > 3):
                                    subpages.append(str(t))
                                    
                        st.write("**Subpages (Identity Related)**")
                        if subpages:
                            # Limited to top 4 sorted subpages
