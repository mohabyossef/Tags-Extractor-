    ...
    if upload_file:
        ...
        if len(df.columns) >= 2:
            ...
            if total_count > 0:
                ...
                # --- CUMULATIVE TAG COLLECTIONS ---
                final_cuisine_tags = set()
                final_normal_tags = set()
                forced_percs = {} # For tags triggered by name

                # CONDITION 1: RESTAURANT NAME PRIORITY
                if res_name:
                ...
                # CONDITION 2: AGGREGATE GROUPS (e.g. Sushi + Ramen = Japanese)
                for target, triggers in cuisine_map.items():
                ...
                with col2:
                    ...
                    st.write(f"Scanned {final_count} unique items. Total clean items for audit: {total_count}")
                    st.divider()
                    ...
                    with c2:
                        ...
                        for tag_name in final_normal_tags:
                            # Priority: Forced Perc (Name/Agg) > Menu Perc > 0
